...
\makebox[0pt][l]{\scriptsize\ttfamily xpath:securityMethod}&
\footnotesize integer[]&
...
\subsection{Changes from PR-20190503}

\begin{itemize}
\item Re-adding a res\_details key for securityMethod ids.
\end{itemize}

\subsection{Changes from PR-20190326}


\begin{itemize}
\item Dropping security\_method\_id again, as it fairly dramatically
complicates interfaces when there's more than one of them.
\item Adding a column authenticated\_only to be used where we have
identified use cases for information from securityMethod. We're still
not re-requiring the corresponding res\_detail key; there are simply no
clear use cases on all of this yet, and there's been enough back and
forth (not to mention that res\_details doesn't really work for
information in interface anyway).
\end{itemize}
...
\item \vorent{securityMethod/@standardID} is now mapped in the interface
