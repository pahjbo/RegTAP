...
\previousversion[http://ivoa.net/documents/RegTAP/20190911]{PR-20190911}
...
At the time of writing, the rules for machine-readable representations
of these vocabularies are only available as a Working Draft
\citep{wd:voc2}; we therefore cannot require the use of these vocabulary
representations at this point.

For RegTAP, these vocabulary resources are relevant because the
...
vocabulary-controlled columns, services MUST translate such deprecated
terms when the vocabularies give replacements.  Until the
machine-readable resources are properly standardised, the required
translations are listed in appendix~\ref{app:voctrans}.
...
once the machine-readable format(s) for the vocabularies have become
normative, overriding appendix~\ref{app:voctrans}.
...
1.1 and VODataService 1.2 use \texttt{vs:}.  
...
\makebox[0pt][l]{\scriptsize\ttfamily }&
 A non-standard extension mimicking glots.\\
...
\rtent{table_utype} is used in data discovery, it should be indexed.
...
spatial coverage using spatial MOCs \citep{2019ivoa.spec.1007F}.  The
...
it must support CONTAINS with an ADQL POINT in the first argument.
...
producing such a relation.  Hence, starting with version 1.2, RegTAP
...
\makebox[0pt][l]{\scriptsize\ttfamily }&
\footnotesize string&
IVOA identifier of the resource this table was taken from (where there is a resource containing this table in its tableset, that resource is preferred over a TAP service).\\

\baselineskip=9pt\relax svcid\hfil\break
\makebox[0pt][l]{\scriptsize\ttfamily }&
\footnotesize string&
IVOA identifier of the TAP service publishing this table\\
...
TAP capabilities,\todo{I'll propose both UDFs for the catalogue, in
which case that would become ivo\_whatever} the above query
...
When this document was written, the policy affected terms from two
...
\sptablerule
\rtent{res_date.value_role}&representative&Collected\\
\rtent{res_date.value_role}&creation&Created\\
\rtent{res_date.value_role}&update&Update\\
\rtent{relationship.relationship_type}&mirror-of&IsIdenticalTo\\
\rtent{relationship.relationship_type}&service-for&IsServiceFor\\
\rtent{relationship.relationship_type}&served-by&IsServedBy\\
\rtent{relationship.relationship_type}&derived-from&IsDerivedFrom\\
\sptablerule
\end{tabular}
\end{inlinetable}
...
\item Requiring ADQL COALESCE and WITH constructs.
\item Including VODataService 1.2 resource types.
\item \rtent{table_name} is no longer case-folded (this is also RegTAP
1.1 Erratum 1).
