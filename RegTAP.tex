...
languages.
The IVOA Registry
Interfaces specification \citep{std:RI1}\todo{update when RI1.1 is REC} defined, among other aspects of
...
\begin{description}
\item[VOResource, v1.1 \citep{TODO}] VOResource sets the foundation for a formal definition of the data
...
mechanisms.
\item[IVOA Identifiers, v2.0 \citep{TODO}]IVOA identifiers are
...
\begin{description}
\item[ADQL, v2.1 \citep{XXXTODO}]The rules for ingestion are designed to allow
...
\vorent{content/contentLevel}, \vorent{content/type},
...
Following the existing practice, minor version changes are not
in\todo{update for schema versioning EN}
...
entails adding\todo{add text on future utype/table-based discovery}
...
\lstinputlisting[language=XML,flexiblecolumns=true,basicstyle=\footnotesize
  ]{makeutypes.xslt}
