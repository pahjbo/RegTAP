...
\rtent{utype}, \rtent{datatype}, \rtent{type\_system}.\todo{Should
extendend\_type be lowercased as well?}
...
Plain \vorent{securityMethod} is an empty element that merely has a
...
be NULL.  A \vorent{securityMethod} element without a \xmlel{standardID}
attribute is also mapped to a \rtent{security\_method\_id} value of
NULL; both cases indicate unauthenticated access.

If an interface supports multiple \vorent{securityMethod}s, RegTAP
ingestors must generate one \rtent{interface} row per security method
(including one with NULL if a \vorent{securityMethod} without
\xmlel{standardID} is present).
